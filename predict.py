...
model_dir = r'/home/admin/workspace/mingzhe/BERT-BiLSTM-CRF-NER/output'
bert_dir = r'/home/admin/workspace/mingzhe/BERT-BiLSTM-CRF-NER/chinese_L-12_H-768_A-12/'
...
def predict_online():
    ...
    def cut(sentence):
        ...
        while (len(sentence) - i) >202:
            for j in range(50):
                if sentence[i+202-j] in ['。', '！', '？', '，', '；','.', ',',';',' ']:
                    sens.append(str(sentence[i:i+202-j])) 
                    break
            i= i+202-j
        sens.append(str(sentence[i:]))
        return sens
    ...
    with graph.as_default():
        new_result = []
        data = pd.read_csv("data_test.csv", encoding="UTF8")
        data["new_model_result"]=range(len(data.loc[:,'detail']))
        new_result=[]
        number = 1
        for sentence1 in data.loc[:,'detail']:
#             print(number)
            per,loc,org,med,fee,occ,dat=[],[],[],[],[],[],[]
            per1,loc1,org1,med1,fee1,occ1,dat1=[],[],[],[],[],[],[]
            number+=1
            sentences = cut(sentence1)  
            for sentence in sentences:
                ...
                per0,loc0,org0,med0,fee0,occ0,dat0 = strage_combined_link_org_loc(sentence, pred_label_result[0])
                for nn in per0:
                    if len(nn.word)>1:
                        per.append(nn.word)
                for nn in loc0:
            ...
            occ=quchong(occ)
            dat=quchong(dat)
            new_result.append({"per":per,"loc":loc,"org":org,"med":med,"fee":fee,"occ":occ,"dat":dat})
        data["new_model_result"] =new_result  # range(len(data.loc[:,'detail'])) #
        data.to_csv('data_test1.csv')
...
def strage_combined_link_org_loc(tokens, tags):
    ...
    per,loc,org,med,fee,occ,dat = eval.get_result(tokens, tags)
    return per,loc,org,med,fee,occ,dat
...
class Result(object):
    ...
    def get_result(self, tokens, tags, config=None):
        ...
        return self.per,self.loc,self.org,self.med,self.fee,self.occ,self.dat
    ...
    def append(self, word, start, end, tag):
        # "B-PER","I-PER","B-LOC","I-LOC","B-ORG","I-ORG","B-MED","I-MED","B-OCC","I-OCC","B-DAT","I-DAT","B-FEE","I-FEE"
        if tag == 'PER':
            self.per.append(Pair(word, start, end, 'PER'))
        elif tag == 'LOC':
            self.loc.append(Pair(word, start, end, 'LOC'))
        elif tag == 'ORG':
            self.org.append(Pair(word, start, end, 'ORG'))
        elif tag == 'MED':
            self.med.append(Pair(word, start, end, 'MED'))
        elif tag == 'OCC':
            self.occ.append(Pair(word, start, end, 'OCC'))
        elif tag == 'DAT':
            self.dat.append(Pair(word, start, end, 'DAT'))
        elif tag == 'FEE':
            self.fee.append(Pair(word, start, end, 'FEE'))
        else:
